...
public class VMProcess extends UserProcess {
	...
	TLBCount++;  //DEBUG
            ...
            if(TLBCount>23&&TLBCount<26){  //DEBUG
	...
	Processor processor = Machine.processor();
        ...
        if(cause==7){
            int regAddr=processor.readRegister(processor.regPC);
            int vppn=processor.pageFromAddress(regAddr);
            int offsetFromAddress=processor.offsetFromAddress(regAddr);
            int pageN=processor.pageSize;
            int dist=pageN-offsetFromAddress;
            int reg3=processor.readRegister(3);
            int reg4=processor.readRegister(4);
	    Lib.debug(w,"Index of PC:\t"+regAddr+"\n");
            Lib.debug(w,"VPN of PC:\t"+vppn+"\n");
            Lib.debug(w,"Distance from page boundary:\t"+dist+"\n");
            Lib.debug(w,"Page size:\t"+pageN+"\n");
            Lib.debug(w,"Register $3 after sb $4 0($3):\t"+reg3+"\n");
            Lib.debug(w,"Register $4 after sb $4 0($3):\t"+reg4+"\n");
        }
	switch (cause) {
