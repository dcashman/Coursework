...
public class UserProcess {
	...
	Lib.debug(w, "Enterign Execute\n");
	  for(int i=0;i<args.length;i++)
	    Lib.debug(w, "name:\t"+name+"\targs:\t"+args[i]+"\n");   //TRACE
		if (!load(name, args)){
	...
	return amount;  */  //AFTER MERGE2 original code above, Pinky's below
        ...
        do{
            ...
            if(!verifyVaddr(pageIndex,false))   //fixing read and write
		return 0;
            ...
            byteNum+= bytesToRead;
            verifyClean(pageIndex);  //DAC DEBUG
    ...
    /*
     *verifyVaddr() - makes sure that the virtual address is accessible
     * params:
     *   vpn - the virtual page to check   
     *   write - true if this is for writeVirtualMemory, false otherwise
     * return true if address is ok, false otherwies
     */
    public boolean verifyVaddr(int vpn, boolean write){
        if (vpn<0||vpn>=numPages){    //changed this to pageTable.numPages*(pageSize-1)(verify) DAC DEBUG  CHANGED FOR VERIFY
	    return false;
	}else{
            return true;
	}
    ...
    /*
     *verifyClean() - makes sure page cleanup happens in write/readVirtualMemory
     * params:
     *   vpn - the virtual page to check   
     * return true if address is ok, false otherwies
     */
	...
	byte[] memory = Machine.processor().getMemory();
        ...
        do{
            pageIndex = Processor.pageFromAddress(vaddr + byteNum);             //get the page it thinks it is on
            //verify the virtual address
            if(!verifyVaddr(pageIndex, true))
		return 0;
            pageOffset=Processor.offsetFromAddress(vaddr+byteNum);    //get offset from page base of this virtual address
            bytesLeftInPage = pageSize- pageOffset;                   //number of bytes you can still write on this page
            ...
            verifyClean(pageIndex);  //shouldn't need to do anything with the value DAC DEBUG
	}while(byteNum<length);
        return byteNum;  //total written
   ...
   return child.pid;
     ...
     ***********************************************************************************************************************************************/
    public int pid;        //CHANGED FOR VMTest
